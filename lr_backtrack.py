...
class LRFinder(object):
    ...
    def track_batch(self, inputs, labels):
        ...
        try:
            self.loss_new_prev = self.loss_new
        except:
            self.loss_new_prev = self.loss
        ...
        # check condition
        self.cond = self.lr_current * self.norm_squared > self.thres and (
                    self.lr_current >= self.lr_min and self.lr_current <= self.lr_max)
